"""
Responds to GET commands by the API
Flask to test API locally
"""
import requests
...
"""
Example usage:
127.0.0.1:5000/team/BOS/stats/pergame/alltime/total
127.0.0.1:5000/team/GSW/stats/perseason/alltime/total
"""
...
def process_team_stats(team, stat_type, time, aggtype, command):
    client = MongoClient(f'mongodb+srv://maxwellbyang04:{MONGO_DB}@nba-api.gajy9.mongodb.net/')
    db = client['nba_stats']
    collection = db['teams']
...
@app.route('/team/<team_name>/stats/<stat_type>/<time>/<aggtype>', methods=['GET'])
def run_api(team_name, stat_type, time, aggtype):
